...
config[c("range_X", "levels_X")] <- format_data(data)[["config"]][c("range_X", "levels_X")]
...
####################################################################################################
##### E-learning estimated working variance
...
data_test <- within(data_generation(within(config, n <- n_test)), { 
  X <- data.frame(X = X, X2 = X^2, X3 = X^3)
